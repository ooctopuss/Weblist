 ...
 */
@MultipartConfig
public class ModListaPServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(idImmagine == -1){
            try {
                idImmagine= listaDao.getByPrimaryKey(idLista).getIdImmagine();
            } catch (DAOException ex) {
                Logger.getLogger(ModListaPServlet.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        
        listaDao.updateListaSpesa(new ListaSpesa(idLista,nome,descrizione,idCategoriaLista,idImmagine));
